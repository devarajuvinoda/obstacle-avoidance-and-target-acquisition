...
class FindTargetSupervisor(SupervisorCSV):
    ...
    def get_reward(self, action):
        ...
        if utils.get_distance_from_target(self.robot,
            ...
            np.savetxt("path_X"+str(val),np.array(ROBOT_POSITION_X))
            np.savetxt("path_Y"+str(val),np.array(ROBOT_POSITION_Y))
            np.savetxt("path_Z"+str(val),np.array(ROBOT_POSITION_Z))
            ROBOT_POSITION_X=[]
            ROBOT_POSITION_Y=[]
            ROBOT_POSITION_Z=[]
            return +500
